...
def read_label(dir, task):
    ...
    Returns:
        labels(list) , node_id_mapping(dict): 
        节点预测
...
def read_file(args, logger):
    ...
    edges = read_edges(directory, node_id_mapping) # [(u1 , v1) , (u2 , v2)] 
    if not di_flag:
        G = nx.Graph(edges)
    else:
        G = nx.DiGraph(edges)
...
def generate_samples_labels_graph(G, labels, task, args, logger):
    """获取数据集，数据大小为总数*args.data_usage

    Args:
        G (图类): 图
        labels (list): 节点的标签信息
        task (string): 当前运行的是哪种任务
        args (类): _description_
...
def get_data_sample(G, set_index, hop_num, feature_flags, max_sprw, label, debug=False):
    ...
    Args:
        ...
        set_index (int): 中心点
        hop_num (int): 邻居层数
        feature_flags (Tuple): (sp , rw) 要获取的特征，sp最短路，rw随机游走
        max_sprw (Tuple): 子图参数，（sp_max , rw_depth)
        label (list): 标签值
    ...
    edge_index = torch.tensor(list(G.edges)).long().t().contiguous() # 2 * shape number_edges
    edge_index = torch.cat([edge_index, edge_index[[1, 0], ]], dim=-1)
    # 变成了一个2 * （number_edges * 2)的矩阵，左边一半和右边一半是上下交换的关系
    # 有向图变无向图
    ...
    # subgrap_node_old_index 表示子图中被选择的节点列表
    # new_edge_index 表示子图中的边(用新编号的点表示)的列表
    # new_set_index 表示 原图起点 映射到子图上是哪个点
    # edge_mask 表示一个长度等于原图边数的True/False列表，如果在子图里对应下标就为True，否则就是False
...
def get_features_sp_sample(G, node_set, max_sp):
    ...
    Args:
        G (networkx): 图
        node_set (numpy.array): 新图中的中心点
        max_sp (int): maximum distance to be encoded for shortest path feature
