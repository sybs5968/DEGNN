...
def read_file():
    ...
    """
    edges , labels , node_id_mapping = [] , [] , dict()
...
def extract_subgaphs(G , labels , set_indices , prop_depth = 1 , layers = 2):
    ...
    for i in tqdm(range(set_indices.shape[0])):
        ...
        edge_index = torch.cat([edge_index , edge_index[[1 , 0] , :]] , dim = -1)
        subgrap_node_old_index , new_edge_index , new_set_index , edge_mask = tgu.k_hop_subgraph(
        torch.tensor(set_indices[i]).long() , hop_num , edge_index , num_nodes = G.number_of_nodes() , relabel_nodes=True)
        num_nodes = subgrap_node_old_index.shape[0]
        new_G = nx.from_edgelist(new_edge_index.t().numpy().astype(dtype=np.int32) , create_using=type(G))
        new_G.add_nodes_from(np.arange(num_nodes , dtype=np.int32))
        ...
        data_list.append(Data(x=x , edge_index=new_edge_index , y=y , set_indices=new_set_index))
    return data_list
...
def get_data(G , labels , size = 1.0):
    ...
    # with open("../myDEGNNtest.txt" , "w") as f:
    #     for data in data_list:
    #         f.write(str(data.x))
    #         f.write(str(data.y))
    #         f.write(str(data.edge_index))
    #         f.write(str(data.set_indices))
    # exit(0)
    ...
    # print(train_mask)
    # exit(0)
    # tmp = train_mask + val_mask + test_mask
    # print(tmp)
    # exit(0)
